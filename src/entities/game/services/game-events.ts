...
class GameEventsService {
	...
	emit(event: GameEvent) {
		if (event.type === 'game-changed') return this.eventsChanel.emit(event.data.id, event);
		if (event.type === 'game-created') return this.eventsChanel.emit('game-created', event);
	}
}
